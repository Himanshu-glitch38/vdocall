...
document.querySelectorAll(".glass:not(#video-chat-area)").forEach((card) => {
  card.addEventListener("mousemove", (e) => {
    ...
    card.style.transform = `perspective(1000px) rotateY(${x * 5}deg) rotateX(${
      y * -5
    }deg)`;
  });
...
const isChatPage =
  window.location.pathname.toLowerCase().endsWith("/chat.html") ||
  window.location.pathname.toLowerCase().endsWith("chat.html");
// let localStream;
...
function setMobileVideoPane(heightPx) {
  ...
  const clamped = Math.min(
    maxHeight,
    Math.max(minHeight, Math.round(heightPx))
  );
...
window.addEventListener("resize", () => {
  if (!videoArea) return;
  if (document.body.classList.contains("in-chat") && isPhoneLayout()) {
    const currentHeight = videoContainer
      ? videoContainer.getBoundingClientRect().height
      : videoArea.clientHeight * 0.4;
...
if (reportBtn) {
  reportBtn.addEventListener("click", () =>
    alert("User reported for violating terms.")
  );
}
